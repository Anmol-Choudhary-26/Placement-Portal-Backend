...
class BasicStats(APIView):
  ...
  def get(self,request):
        ...
        if jtype == "intern":
            topCompaniesIntern = Interned.objects.filter(job_role__drive__session = session).values(name = F('job_role__drive__company__name')).annotate(max_stipend = Max('job_role__ctc')).order_by('-job_role__ctc')#student__student__passing_year 
            topCompaniesOffCampus = Offcampus.objects.filter(session = session,type = 'intern').values(name = F('company__name')).annotate(max_stipend = Max('ctc'))
            ...
            topCompaniesPlacement = Placed.objects.filter(job_role__drive__session = session).values(name = F('job_role__drive__company__name')).annotate(max_ctc = Max('job_role__ctc'))
            topCompaniesOffCampus = Offcampus.objects.filter(session = session,type = 'placement').values(name = F('company__name')).annotate(max_ctc = Max('ctc'))
            topCompaniesPPO = PPO.objects.filter(session = session).values(name = F('company__name')).annotate(max_ctc = Max('ctc'))
            ...
            # course_name = B.Tech.
            oncampus_data = Placed.objects.filter(job_role__drive__session = session,student__student__course__name = 1).values(roll =F('student__student'),branch = F('student__student__branch__branchName'),ctc_ = F('job_role__ctc'))
            offcampus_data = Offcampus.objects.filter(session = session,type = 'placement',student__course__name = 1).values(roll=F('student'),branch = F('student__branch__branchName'),ctc_=F('ctc'))
            ppo_data = PPO.objects.filter(session = session,student__course__name = 1).values(roll=F('student'),branch = F('student__branch__branchName'),ctc_=F('ctc'))

            complete_data = oncampus_data.union(offcampus_data,ppo_data) 
            offers = complete_data.count()
            print(offers)
            
            branches = Specialization.objects.filter(course__name = 1).values_list('branchName',flat=True)
            complete_data = pd.DataFrame(complete_data)
            temp = 0
            course_wise = []
